...
class YCBEvaluator {
 private:
  ...
  static constexpr icg::Intrinsics kYCBIntrinsics{
 ...
 private:
  ...
  void ResetBodies(const std::shared_ptr<icg::Tracker> &tracker_ptr,
  ...
  void ExecuteMeasuredTrackingCycle(
      const std::shared_ptr<icg::Tracker> &tracker_ptr, int load_index,
      int iteration, ExecutionTimes *execution_times) const;
  void UpdateCameras(const std::shared_ptr<icg::Tracker> &tracker_ptr,
  ...
  void CalculatePoseResults(const std::shared_ptr<icg::Tracker> tracker_ptr,
  ...
  std::function<void(std::shared_ptr<icg::Tracker>)> tracker_setter_{
      [](auto) {}};
  std::function<void(std::shared_ptr<icg::Refiner>)> refiner_setter_{
      [](auto) {}};
...
#endif  // ICG_EXAMPLES_YCB_EVALUATOR_H_
