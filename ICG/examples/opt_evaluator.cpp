  ...
  *tracker_ptr = std::make_shared<icg::Tracker>("tracker");
  ...
  if (use_region_modality_) {
    ...
    auto color_camera_ptr{std::make_shared<icg::LoaderColorCamera>(
        "color_camera", camera_directory, kOPTIntrinsics, "", 1, 4)};
  ...
  if (use_depth_modality_) {
    // Init camera
    std::filesystem::path camera_directory{
        dataset_directory_ / "3D" / run_configuration.sequence_name / "depth"};
    auto depth_camera_ptr{std::make_shared<icg::LoaderDepthCamera>(
        "depth_camera", camera_directory, kOPTIntrinsics, 0.001f, "", 1, 4)};
    depth_camera_ptr->set_camera2world_pose(
        icg::Transform3fA{Eigen::Matrix4f{kDepth2Color_Pose.data()}});
...
void OPTEvaluator::CalculatePoseResults(
    const std::string &body_name, const icg::Transform3fA &body2world_pose,
    const icg::Transform3fA &gt_body2world_pose, Result *result) const {
  float error = 0.0f;
  icg::Transform3fA delta_pose{
