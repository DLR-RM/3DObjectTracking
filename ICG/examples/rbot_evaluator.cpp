...
void RBOTEvaluator::EvaluateRunConfiguration(
  ...
  const auto &body_ptr{
      tracker_ptr->optimizer_ptrs()[0]->modality_ptrs()[0]->body_ptr()};
...
void RBOTEvaluator::SetUpTracker(
  ...
  *tracker_ptr = std::make_shared<icg::Tracker>("tracker");
  ...
  auto body_ptr{std::make_shared<icg::Body>(*body2body_ptr_map_.at(body_name))};
  renderer_geometry_ptr->AddBody(body_ptr);

  // Init region modality
  auto region_modality_ptr{std::make_shared<icg::RegionModality>(
      body_name + "_modality", body_ptr, camera_ptr,
  ...
  if (run_configuration.occlusions) {
    // Init occlusion body
    auto occlusion_body_ptr{std::make_shared<icg::Body>(
        *body2body_ptr_map_.at(kOcclusionBodyName))};
    renderer_geometry_ptr->AddBody(occlusion_body_ptr);

    // Init occlusion region modality
    auto occlusion_region_modality_ptr{std::make_shared<icg::RegionModality>(
        kOcclusionBodyName + "_modality", occlusion_body_ptr, camera_ptr,
        body2model_ptr_map_.at(kOcclusionBodyName))};
    region_modality_setter_(occlusion_region_modality_ptr);
    occlusion_region_modality_ptr->SetUp();

    // Init occlusion optimizer
    auto occlusion_optimizer_ptr{
        std::make_shared<icg::Optimizer>(kOcclusionBodyName + "_optimizer")};
    occlusion_optimizer_ptr->AddModality(occlusion_region_modality_ptr);
    optimizer_setter_(occlusion_optimizer_ptr);
    occlusion_optimizer_ptr->SetUp();
    (*tracker_ptr)->AddOptimizer(occlusion_optimizer_ptr);

    // Init depth renderer main body
    auto depth_renderer_ptr{std::make_shared<icg::FocusedBasicDepthRenderer>(
...
void RBOTEvaluator::ResetBody(const std::shared_ptr<icg::Tracker> &tracker_ptr,
                              int i_frame) const {
  tracker_ptr->optimizer_ptrs()[0]
      ->modality_ptrs()[0]
      ->body_ptr()
      ->set_body2world_pose(poses_gt_first_[i_frame]);
  tracker_ptr->optimizer_ptrs()[0]->modality_ptrs()[0]->StartModality(0, 0);
}

void RBOTEvaluator::ResetOcclusionBody(
    const std::shared_ptr<icg::Tracker> &tracker_ptr, int i_frame) const {
  tracker_ptr->optimizer_ptrs()[1]
      ->modality_ptrs()[0]
      ->body_ptr()
      ->set_body2world_pose(poses_gt_second_[i_frame]);
  tracker_ptr->optimizer_ptrs()[1]->modality_ptrs()[0]->StartModality(0, 0);
}
...
void RBOTEvaluator::CalculatePoseResults(
    const icg::Transform3fA &body2world_pose,
    const icg::Transform3fA &body2world_pose_gt, Result *result) const {
...
void RBOTEvaluator::VisualizeFrameResult(const Result &result,
  ...
  std::cout << title << ": "
            ...
            << " us "
            << "rotation_error = " << result.rotation_error * 180.0f / icg::kPi
            << ", "
...
bool RBOTEvaluator::ReadPosesRBOTDataset(
    const std::filesystem::path &path, std::vector<icg::Transform3fA> *poses) {
