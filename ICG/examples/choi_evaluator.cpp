...
#pragma omp critical
  ...
  final_results_.insert({"average", final_result_average});
...
bool ChoiEvaluator::EvaluateBody(
  ...
  std::vector<icg::Transform3fA> gt_body2world_pose;
...
void ChoiEvaluator::CalculateAverageSequenceResult(SequenceResult *result) {
  ...
  for (const auto &frame_result : result->frame_results) {
    result->rms_error_x += icg::square(frame_result.error_x);
    result->rms_error_y += icg::square(frame_result.error_y);
    result->rms_error_z += icg::square(frame_result.error_z);
    result->rms_error_alpha += icg::square(frame_result.error_alpha);
    result->rms_error_beta += icg::square(frame_result.error_beta);
    result->rms_error_gamma += icg::square(frame_result.error_gamma);
    result->mean_execution_times.calculate_correspondences +=
...
void ChoiEvaluator::CalculatePoseResults(
    ...
    const icg::Transform3fA &gt_body2world_pose, Result *result) const {
  ...
  result->error_alpha = rpy_error.x() * 180.0f / icg::kPi;
  result->error_beta = rpy_error.y() * 180.0f / icg::kPi;
  result->error_gamma = rpy_error.z() * 180.0f / icg::kPi;
}
...
bool ChoiEvaluator::GetGTPosesChoiDataset(
  ...
  for (std::string line; std::getline(ifs, line);) {
    icg::Transform3fA pose{icg::Transform3fA::Identity()};
