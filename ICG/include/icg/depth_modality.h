 ...
 * gradient vector and Hessian matrix that are used by the \ref Optimizer to
 * update the \ref Body pose.
 *
 * \details The modality is able to measure occlusions using the depth image or
 * model occlusions using renderings from a \ref FocusedDepthRenderer object.
 *
 ...
 * @param n_points number of correspondence points.
 ...
 * iteration. If fewer values than `n_corr_iterations` are given, the last
 * provided standard deviation is used.
...
class DepthModality : public Modality {
 ...
 private:
  ...
  bool IsPointValid(const DataPoint &data_point, bool measure_occlusions,
                    bool model_occlusions) const;
  ...
  void DrawFocusedPoints(const cv::Vec3b &color_point, cv::Mat *image) const;
  ...
  float considered_distance_square_{};
...
#endif  // ICG_INCLUDE_ICG_DEPTH_MODALITY_H_
